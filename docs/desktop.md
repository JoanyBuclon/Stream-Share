...
| **pnpm 11 refuse les scripts d'install** : `esbuild` échouait ; `electron-winstaller` (tiré par electron-builder 26) aussi. Le diagnostic d'origine (« `allowBuilds` ne vit que dans `pnpm-workspace.yaml`, que `--ignore-workspace` ignore ») n'est plus la raison : `electron/` a désormais son propre `pnpm-workspace.yaml` — ajouté pour porter les `overrides` de sécurité — et `--ignore-workspace` a disparu des appels. `allowBuilds` **pourrait** donc y vivre ; on garde `--ignore-scripts` parce que rien ici n'a besoin d'un script de cycle de vie (Electron télécharge son binaire au premier lancement, electron-builder sa propre toolchain). | `esbuild-wasm` + `--ignore-scripts` à l'install                      |
