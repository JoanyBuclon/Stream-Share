...
struct DisplayHdr {
  std::wstring device_name;  // \\.\DISPLAY1 — matches what EnumDisplayDevices and Electron report
  bool hdr;
  ...
  // hardcoded to 80 or 200 the way most samples do.
...
std::vector<DisplayHdr> QueryDisplays() {
  ...
  for (UINT a = 0;; ++a) {
    ...
    for (UINT o = 0;; ++o) {
      ...
      d.sdr_white_nits = 80.0f;  // replaced below when the OS exposes the user's setting
